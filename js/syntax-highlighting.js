...
function applySyntaxHighlighting(codeElement) {
    ...
    code = code.replace(/#(.*)$/gm, (match, content) => {
        // Only highlight comments that contain actual terminal error messages (lowercase "error:" with word boundary)
        return createToken('comment', '#' + content);
    });

    // Error keywords (only the word "error" before colon)
    code = code.replace(/\berror(?=:)/g, (match) => {
        return createToken('error-message', match);
    });

    // Error underlines (tildes and carets)
    code = code.replace(/(~+|\^+)/g, (match) => {
        return createToken('error-underline', match);
    });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (heroCode) {
        ...
        const preElement = heroCode.parentElement;
        ...
        function typeCode() {
            if (currentCharIndex < originalCode.length) {
                ...
                // Animation complete, fade in the badge after a short delay
                setTimeout(() => {
                    preElement.classList.add('badge-visible');
                }, 500);
            }
        }
